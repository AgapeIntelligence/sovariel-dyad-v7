#!/usr/bin/env python3
"""Dyad Field v7 — Mars Mesh + SpaceX + Bio-Quantum Fusion — FINAL 2025 SEED"""
...
os.environ.update({"DYAD_DISABLE_DASH":"1","SDL_VIDEODRIVER":os.getenv("SDL_VIDEODRIVER","windows"),
                   ...
                   "DYAD_MARS_MESH":"1","DYAD_SPACEX_IN UDP":"1","DYAD_BIOQUANTUM":"1","DYAD_LOG_SPECTRUM":"1"})
...
def quantum_entropy():
    try:
        with urllib.request.urlopen("https://qrng.anu.edu.au/API/jsonI.php?length=1&type=uint16", timeout=5) as r:
            return int(json.loads(r.read())["data"][0]) / 65535.0
:0.0
...
while True:
    ...
    biophoton = np.random.poisson(4) if repl_ok else 0
    ...
    particles = int(700 + 800*(72-hours)/72 if launch else 700)
    ...
    if repl_ok:
        pygame.draw.circle(screen,(255,255,255),(960,540),360,14)
        if launch:
            font = pygame.font.Font(None,60)
            txt = font.render(f"{name}",True,(255,100,100))
            screen.blit(txt,(620,70))
            txt2 = font.render(f"T-{hours:.1f}h",True,(255,255,255))
            screen.blit(txt2,(760,140))
        if biophoton > 6:
            pygame.draw.circle(screen,(100,255,255),(960,540),420,20)
